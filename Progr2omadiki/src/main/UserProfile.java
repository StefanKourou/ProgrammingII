...
public class UserProfile {
	...
	/**
	 * logged-in user, all methods are responsible for retrieving data from this
	 * user
	 */
	static String loggedUsername;
	...
	public static void showProfile() {
		String sql1 = "SELECT Email, Discoverable, UserKeywords " + "FROM Users " + "WHERE Username = '"
				+ loggedUsername + "'";
	...
	public static void showMsgCount() {
		int msgs;
		String sql = "SELECT COUNT(MsgID) " + "FROM Messages " + "WHERE MsgUsername ='" + loggedUsername + "'";
	...
	public static void showTimeLoggedIn() {
		String timeLogged;
		String sql = "SELECT (strftime('%M', 'now') - strftime('%M',  LastLoginTime)) as timeLogged " + "FROM Users "
	...
	/**
	 * Shows the Reactions that the user has given and has been given with a
	 * graphical way
	 */
	public static void showReactions() {
		...
		String sqltest = "SELECT COUNT(Reaction) " + "FROM Reactions " + "WHERE ReUsername ='" + loggedUsername + "'";
		try (Statement stmt = conn.createStatement();) {
			ResultSet rs = stmt.executeQuery(sqltest);
			// if this user has reacted to at least 1 msg, show the reactions to the screen
			if (rs.getInt(1) > 0) {
				String sql = "SELECT Reaction " + "FROM Reactions " + "WHERE ReUsername ='" + loggedUsername + "'";
				rs = stmt.executeQuery(sql);
				while (rs.next()) {
					int re = rs.getInt("Reaction");
					switch (re) {
					case 1:
						likesUserGave++;
						break;
			...
			System.out.println("You have:" + "\n\t" + "liked " + likesUserGave + " message(s)" + "\n\t" + "disliked "
					+ dislikesUserGave + " message(s)" + "\n\t" + "and loved " + heartsUserGave + " message(s)");
			// check if this user has any msgs that have been reacted to
			sqltest = "SELECT COUNT(ReMsg) " + "FROM Reactions " + "WHERE ReMsg in (SELECT MsgID " + "FROM Messages "
					+ "WHERE MsgUsername ='" + loggedUsername + "')";
			rs = stmt.executeQuery(sqltest);
			// if this user has at least 1 msg that has been reacted to, show the reactions
			// to the screen
			if (rs.getInt(1) > 0) {
				String sql = "SELECT Reaction " + "FROM Reactions " + "WHERE ReMsg in (SELECT MsgID " + "FROM Messages "
						+ "WHERE MsgUsername ='" + loggedUsername + "')";
				rs = stmt.executeQuery(sql);
				while (rs.next()) {
					int re = rs.getInt("Reaction");
	...
	public static String getLoginTime() {
		String logintime;
		String sql = "SELECT LastLoginTime " + "FROM Users " + "WHERE Username ='" + loggedUsername + "'";
		try (Statement stm = conn.createStatement(); ResultSet rs = stm.executeQuery(sql)) {
	...
	public static String getLogoutTime() {
		String logouttime;
		String sql = "SELECT LastLogoutTime " + "FROM Users " + "WHERE Username ='" + loggedUsername + "'";
		try (Statement stm = conn.createStatement(); ResultSet rs = stm.executeQuery(sql)) {

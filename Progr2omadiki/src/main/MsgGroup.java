...
public class MsgGroup extends User {
	 ...
	 * 
	 * @param ad takes a binary number(0 or 1) so it can know how to calculate the
	 *           said group (to know if the group has just been created or not)
	 */
				...
				if (checkExistingUser(name)) {
					...
					if (queryresult.getInt(1) == 1) {
						if (ad == 1) {
							...
							String sql1 = "SELECT MAX(MsgGroupID) " + "FROM MsgGroups " + "WHERE MsgGroupCreator='"
									+ loggedUsername + "'";
	...
	public void showMsgGroups() {
		...
		String sqltest = "SELECT COUNT(DISTINCT MsgGroupID) FROM MsgGroups, GroupUsersRelations "
				...
				+ "(MsgGroups.MsgGroupID = GroupUsersRelations.RelMsgGroup " + "AND GroupUsersRelations.RelUsername='"
				+ loggedUsername + "')";
		try (Statement stmt = conn.createStatement();) {
			...
			if (rs.getInt(1) > 0) {
				String sql = "SELECT Distinct MsgGroupID, MsgGroupName " + "FROM MsgGroups, GroupUsersRelations "
	...
	/**
	 * shows all MessageGroups of the logged in user that have new msgs, alongside
	 * said msgs
	 */
	...
	public void showNewMessages() {
		...
		String sqltest = "SELECT COUNT(MsgGroupID) " + "FROM Messages, MsgGroups, GroupUsersRelations, Users "
				+ "WHERE Messages.MsgCreationTime > Users.LastLogoutTime AND "
				+ "Messages.MsgGroup = MsgGroups.MsgGroupID AND " + "Users.Username = '" + loggedUsername + "' "
				+ "AND GroupUsersRelations.RelMsgGroup = MsgGroups.MsgGroupID "
				+ "AND GroupUsersRelations.RelUsername = Users.Username " + "GROUP BY MsgGroupID "
				+ "HAVING MAX(MsgID)";
		try (Statement stmt = conn.createStatement();) {
			Message ms = (Message) this;
			...
			clearScreen();
			ResultSet rs = stmt.executeQuery(sqltest);
			// if the user has at least 1 new message, show all of them to the screen with
			// the group
			if (rs.getInt(1) > 0) {
				...
				rs = stmt.executeQuery(sql);
				while (rs.next()) {
					System.out.println(rs.getInt("MsgGroupID") + "\t" + rs.getString("MsgGroupname") + "\t"
	...
	public boolean checkGroupUserExists(int groupID) {
		...
		String sql = "SELECT COUNT(*) " + "FROM GroupUsersRelations " + "WHERE RelMsgGroup =" + groupID + " "
